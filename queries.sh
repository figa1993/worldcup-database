...
echo "$($PSQL "SELECT (SUM(winner_goals)+SUM(opponent_goals))/COUNT(winner_goals)::numeric FROM games")"
...
echo "$($PSQL "SELECT FORMAT('%s|%s',year,name) FROM teams INNER JOIN games on winner_id=team_id WHERE round='Final' ORDER BY year")"
